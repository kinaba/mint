#ifndef KMONOS_NET_MINT_TREAP_INDEX_HPP
#define KMONOS_NET_MINT_TREAP_INDEX_HPP
...
template<typename Priority, typename Compare, typename SuperMeta, typename TagList>
class treap_index
	: protected SuperMeta::type
{
	...
	// 以下のコメントで
	//   [Impl]     は、今回の treap_set 用インデックスのための実装詳細
...
protected:
	...
	typedef intrusive_hook_node<
		boost::intrusive::bs_set_member_hook<>, typename super::node_type, Priority
	> node_type;
...
private:
	// [Impl]
	//    intrusive の treap_set を使った内部実装

	typedef boost::intrusive::treap_set<node_type,
		boost::intrusive::member_hook<node_type,
			boost::intrusive::bs_set_member_hook<>, &node_type::hook_>,
...
public:
	// [Custom]
	//   コンストラクタとデストラクタとコピー等
	treap_index( const treap_index& rhs ) : super(rhs) {} // あとで copy_ が呼ばれる
	treap_index( const ctor_args_list& args_list, const allocator_type& al )
...
public:
	...
	void  swap(treap_index& x)    {this->final_swap_(x.final());}
...
void swap( treap_index<P,C,S,T>& lhs, treap_index<P,C,S,T>& rhs )
{
	lhs.swap(rhs);
}
